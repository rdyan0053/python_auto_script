"""
    爬取壁纸图片
    网址：https://pic.netbian.com/4kmeinv/index.html
"""
...
for page in range(1, 6):
    ...
    for li in li_list:
        ...
        img_title = li.xpath('./a/b/text()')[0] + '.jpg'  # 左侧./表示xpath的调用者对应的标签（加点.就是在做局部解析，不加就是在做全局解析）
        img_src = 'https://pic.netbian.com' + li.xpath('./a/img/@src')[0]

        # 对图片发起请求，存储图片数据
        img_data = requests.get(url=img_src, headers=headers).content  # 得到的是二进制数据
        # girls/123.jpg
        img_path = dirName + '/' + img_title
        with open(img_path, 'wb') as fp:  # w是写入的意思，b是二进制的含义
            fp.write(img_data)
